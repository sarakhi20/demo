...
const prevProject = ()=>{
  ...
  return (
    <motion.div initial={{opacity:0, x:-200}} transition={{duration:1}} whileInView={{opacity:1, x:0}} viewport={{once:true}} className='container mx-auto py-4 pt-20 px-6 md:px-20 lg:px-32 my-20 w-full overflow-hidden' id='Projects'>
        ...
        <p className='text-gray-500 max-w-80 text-center mb-8 mx-auto'>Crafting Space, Building Ledacies-Explore Our Portfolio</p>
          ...
          >
            {projectsData.map((project,index)=>(
              <div key={index} className='relative flex-shrink-0 w-full sm:w-1/4'>
                ...
                <div className='absolute left-0 right-0 bottom-5 flexbjustify-center'>
                  <div className='inline-block bg-white w-3/4 px-4 py-2 shadow-md'>
                  <h2 className='text-xl font-semibold text-gray-800'>
                    {project.title}
                  </h2>
                  <p className='text-gray-500 text-sm'>
                    {project.price} <span className='px-3'></span> {project.location}
                  </p>
                  </div>
                </div>
              </div>
